...
void WindowHandle::init(const std::string_view& title, int width, int height, WindowHandle* parent,
                        int opts)
{
    KY_CONDITION_FATAL(title[title.size()] == '\0', "Title string must be null terminated");
    KY_CONDITION_FATAL(title[title.size()] == '\0', "Title string must be null terminated");
    KY_CONDITION_FATAL(!(opts & WINDOW_HANDLE_FULLSCREEN_BIT && opts & WINDOW_HANDLE_WINDOWED_BIT),
    ...
    // glfwWindowHint(GLFW_DECORATED, (opts & ~WINDOW_HANDLE_BORDERLESS_BIT) !=
    // 0); glfwWindowHint(GLFW_TRANSPARENT_FRAMEBUFFER,
    ...
    KY_CONDITION_FATAL(glfw_handle != nullptr, "Failed to create GLFW window");
...
{
    KY_CONDITION_ERROR(glfw_handle != nullptr, "WindowHandle is not valid");
...
{
    KY_CONDITION_FATAL(glfwInit(), "Failed to initialize GLFW");
